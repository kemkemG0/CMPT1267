...
void Game::ProcessInput()
{
	...
	if (state[SDL_SCANCODE_I] || state[SDL_SCANCODE_UP])
	{
		mPaddle2Dir -= 1;
	}
	if (state[SDL_SCANCODE_K] || state[SDL_SCANCODE_DOWN])
	{
		mPaddle2Dir += 1;
	}
}
...
void Game::UpdateGame()
{
	...
	float diffLeft = mPaddlePos.y - mBallPos.y;
	...
	float diffRight = mPaddlePos2.y - mBallPos.y;
	diffRight = (diffRight > 0.0f) ? diffRight : -diffRight;
	if (diffRight <= paddleH / 2.0f &&
